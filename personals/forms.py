...
class PersonalForms(forms.ModelForm):
    ...
    first_name = forms.CharField(
        label="First Name", widget=forms.TextInput(attrs={"placeholder": "Joe"})
    )
    last_name = forms.CharField(
        label="Last Name", widget=forms.TextInput(attrs={"placeholder": "Doe"})
    )
    photo = forms.ImageField(label="Your photography")
    email = forms.EmailField(
        label="Email", widget=forms.EmailInput(attrs={"placeholder": "joe@doe.com"})
    )
